...
class TokenAuthenticator<T : AuthToken>(
    ...
    override fun authenticate(route: Route?, response: Response): Request? {
        ...
        val oldToken = CredentialsAuth.getToken<T>()

        synchronized(this) {
            // Prevent multiple unnecessary refresh token calls
            val syncedToken = CredentialsAuth.getToken<T>()
            ...
            return runBlocking {
                try {
                    val newToken = CredentialsAuth.refreshToken<T>()?.apply {
                    ...
                    CredentialsAuth.saveToken(newToken)
